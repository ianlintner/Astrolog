...
class Game {
    ...
    generateStarSystems() {
        const systemNames = [
            ...
            'Phi', 'Chi', 'Psi', 'Omega', 'Andromeda', 'Centauri', 'Proxima', 'Sirius', 'Vega', 'Rigel',
            'Betelgeuse', 'Antares', 'Aldebaran', 'Spica', 'Pollux', 'Fomalhaut', 'Deneb', 'Regulus', 'Adhara', 'Castor'
        ];
        ...
        for (let i = 0; i < 100; i++) {
            ...
            const suffix = i >= systemNames.length ? ` ${suffixes[Math.floor(i / systemNames.length) % suffixes.length]}` : '';
    ...
    generateHyperLanes() {
        ...
        for (let i = 0; i < this.starSystems.length; i++) {
            ...
            for (let j = 0; j < connectCount; j++) {
                ...
                const exists = this.hyperLanes.some(lane => 
    ...
    assignSystemEconomies() {
        ...
        this.starSystems.forEach(system => {
            ...
            const importPool = this.goodsTypes.filter(g => 
                !system.exports.find(e => e.id === g.id)
            );
    ...
    setupEventListeners() {
        ...
        document.getElementById('btn-buy-warehouse').addEventListener('click', () => this.buyWarehouse());
        document.getElementById('btn-buy-vehicle').addEventListener('click', () => this.buyVehicle());
        document.getElementById('btn-buy-slot').addEventListener('click', () => this.buyPortSlot());
        document.getElementById('btn-create-route').addEventListener('click', () => this.createRoute());
    }
